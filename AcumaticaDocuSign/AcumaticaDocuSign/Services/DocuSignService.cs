...
namespace AcumaticaDocuSign
{
    ...
    class DocuSignService
    {
        ...
        /// </summary>
        public static string Authenticate(DocuSignAccount account)
        {
            var model = new BaseRequestModel { DocuSignAccount = account };
        ...
        /// <param name="account">Docusign account<see cref="DocuSignAccount"></see> </param>
        /// </summary>
        public static LoginAccount AuthenticateUser(DocuSignAccount account)
        {
            var model = new BaseRequestModel { DocuSignAccount = account };
            ...
            where TArgs : BaseRequestModel
        {
            ...
            catch (ApiException exception)
            {
                ...
                {
                    var accountId = Authenticate(args.DocuSignAccount);
                    ...
                    var message = string.Format(Messages.DocuSignEnvelopeLock, time.ToShortTimeString());
        ...
        private static Func<BaseRequestModel, GetEnvelopeHistoryResponseModel> GetEnvelopeHistoryFunc()
        {
            return (request) =>
            {
                var accountId = Authenticate(request.DocuSignAccount);
                var envelopesApi = new EnvelopesApi();
        ...
        private Func<VoidEnvelopeRequestModel, UpdateEnvelopeResponseModel> VoidDraftEnvelopeFunc()
        {
            return (request) =>
            {
                var account = AuthenticateUser(request.DocuSignAccount);
        ...
        private static Func<BaseRequestModel, string> AuthenticateFunc()
        {
            return (request) =>
            {
                Configuration.Default.ApiClient = request.DocuSignAccount.IsTestApi.HasValue && request.DocuSignAccount.IsTestApi.Value
                    ? new ApiClient(request.DocuSignAccount.ApiUrl)
                    : new ApiClient();

                Configuration.Default.DefaultHeader.Clear();
                Configuration.Default.AddDefaultHeader("X-DocuSign-Authentication", GetAuthHeader(request.DocuSignAccount));
                 ...
                 */
                if (!request.DocuSignAccount.IsTestApi.HasValue || !request.DocuSignAccount.IsTestApi.Value)
                {
                    string baseURL = (loginInfo.LoginAccounts.FirstOrDefault() != null) ? loginInfo.LoginAccounts.FirstOrDefault().BaseUrl : null;
                    if (!String.IsNullOrEmpty(baseURL))
                    {
                        string sLookFor = "restapi";
        ...
        private Func<BaseRequestModel, ViewUrl> RedirectFunc()
        {
            return (request) =>
            {
                ...
                {
                    ReturnUrl = request.DocuSignAccount.IsTestApi.Value
        ...
        private static string GetAuthHeader(DocuSignAccount account)
        {
        ...
        private static Notification CreateNotification(DocuSignEnvelopeInfo envelope)
        {
        ...
        private static IEnumerable<CarbonCopy> ParseCarbonCopy(IEnumerable<DocuSignRecipient> recipients, DocuSignEnvelopeInfo envelopeInfo)
        {
        ...
        private static IEnumerable<Signer> ParseSigners(IEnumerable<DocuSignRecipient> recipients, DocuSignEnvelopeInfo envelopeInfo)
        {
            var tabs = new Tabs();
